...
Graph::~Graph()
{
  ...
  for (std::list<Edge *>::iterator it(edgeList_.begin()); it != edgeList_.end(); ++it)
  {
    if (*it != 0)
    {
      delete *it;
...
GraphStatus Graph::buildNodes()
{
  ...
  Node *n1(nullptr);
  Node *n2(nullptr);
  for (std::map<std::pair<unsigned int, unsigned int>, struct PathDatabase::PathDatabaseType>::iterator it(path_.db_.begin()); it != path_.db_.end(); ++it)
  {
  ...
  Node *n(nullptr);
  for (std::map<unsigned int, struct LocationDatabase::LocationDatabaseType>::iterator it(loc_.db_.begin()); it != loc_.db_.end(); ++it)
  {
...
GraphStatus Graph::finishBuild()
{
  ...
  for (std::map<std::tuple<unsigned int, unsigned int, TransportMode>, Edge *>::iterator it(edges_.begin()); it != edges_.end(); ++it)
  {
    src = (it->second)->src();
    dest = (it->second)->dest();
  ...
  try
  {
    for (std::map<std::pair<unsigned int, TransportMode>, Node *>::iterator it(nodes_.begin()); it != nodes_.end(); ++it)
    {
